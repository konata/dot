...
export default recipe("code", "Visual Studio Code.app", "Code/User",
  ["settings.json"],
  {
    available: context => context.app() && context.command("code"),
    async save(context) {
      const extensions = (await context.output("code", ["--list-extensions"]))
        .split(/\r?\n/)
        .map(line => line.trim())
        .filter(Boolean)
        .sort()
    ...
    async restore(context) {
      for (const extension of await context.lines("extensions.txt")) {
        await context.run("code", ["--install-extension", extension])
      }
    },
    async ["@save"](context) {
